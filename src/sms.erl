...
construct_octets([H|T], Padd_Index) when is_integer(Padd_Index) == true ->
  <<Rev_H>> = binary:encode_unsigned(binary:decode_unsigned(<<H>>, little)),
  construct_octets(T, <<0:Padd_Index, Rev_H:7>>);
construct_octets([H|T], Res) ->
  <<Rev_H>> = binary:encode_unsigned(binary:decode_unsigned(<<H>>, little)),
  construct_octets(T, <<Res/bitstring, Rev_H:7>>);
...
start() ->
  ...
  io:format("SMS (length ~p - spare ~p): ~w~n", [Len_SMS, length(Str) rem 8, SMS]).
